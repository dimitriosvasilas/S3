...
const propagateLog = require('./utilities/propagateLog')
...
const services = {
    ...
    */
    ...
    metadataStoreObject(bucketName, dataGetInfo, cipherBundle, params, cb) {
        ...
        metadataValidateBucketAndObj(mdValParams, log, (err, bucket, objMD) => {
            console.log()
            var prevVersion = !objMD ? objMD :
            {
                contentLength: objMD['content-length'],
                lastModified: new Date(objMD['last-modified']).getTime(),
                attributes: {
                    contentType: objMD['content-type'],
                    ownerDisplayName: objMD['owner-display-name'],
                }
            };
            for (var key in objMD) {
              if (key.startsWith('x-amz-meta-') && !key.startsWith('x-amz-meta-s3cmd')) {
                  prevVersion.attributes[key] = objMD[key]
              }
            }
            ...
            return async.waterfall([
                ...
                var updateNotification = {
                    ...
                    old_state: prevVersion
                }
                for (var key in metaHeaders) {
                    if (key != 'x-amz-meta-s3cmd-attrs') {
                        updateNotification.new_state.attributes[key] = metaHeaders[key]
                    }
                }
                propagateLog.emit(updateNotification, err, { contentMD5, versionId }, cb);
            });
        });
    },
        ...
        function deleteMDandData() {
            return metadata.deleteObjectMD(bucketName, objectKey, options, log,
                (err, res) => {
                    ...
                    for (var key in objectMD) {
                      if (key.startsWith("x-amz-meta")) {
                        updateNotification.old_state.attributes[key] = objectMD[key]
                      }
                    }
                    ...
                    deleteLog, err => {
                        ...
                        propagateLog.emit(updateNotification, null, res, cb);
                    });
                });
        }
