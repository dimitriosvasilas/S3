...
function isConfigRegion(config, locationConstraint, region) {
    if (config.locationConstraints[locationConstraint] &&
      config.locationConstraints[locationConstraint].information &&
      config.locationConstraints[locationConstraint].information.region &&
      config.locationConstraints[locationConstraint].information.region
       === region) {
        return true;
    }
...
export function createBucket(authInfo, bucketName, headers,
    ...
    (err, results) => {
        ...
        // For old behavior:
        // 1) new locationConstraint should either be undefined or us-east-1
        ...
        if ((!locationConstraint ||
            isConfigRegion(config, locationConstraint, 'us-east-1')) &&
            (!existingBucketMD.getLocationConstraint() ||
            isConfigRegion(config, existingBucketMD.getLocationConstraint(),
            'us-east-1')) && usEastBehavior) {
