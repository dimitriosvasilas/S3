...
import { unsupportedQueries } from './routes/routesUtils';
...
function checkUnsuportedRoutes(req, res, log) {
    const unsupportedRequestQuery = Object.keys(req.query)
        .some(key => (unsupportedQueries.indexOf(key) > -1));
    if (unsupportedRequestQuery) {
