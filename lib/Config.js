    ...
    _getConfig() {
        ...
        if (config.listenOn !== undefined) {
            ...
            config.listenOn.forEach(item => {
                ...
                // XXX TODO FIXME
                //
                // Since we want to accept FQDNs for the listen-on and not
                // necessarily only IP addresses, we need to disable this check
                // until we actually resolve the hostnames before setting up
                // the final listenOn list.
                // In the long run, we could resolve the FQDNs into IPs to
                // ensure they're valid, or simply forget about checking,
                // discharging this role onto the administrator.
                //
