...
class Config extends EventEmitter {
    ...
    _getConfig() {
        ...
        this.logPropd = {};
        if (config.logPropd) {
            if (config.logPropd.port !== undefined) {
                assert(Number.isInteger(config.logPropd.port) &&
                    config.logPropd.port > 0,
                    'bad config: logPropd port must be a positive integer');
                this.logPropd.port = config.logPropd.port;
            }
            if (config.logPropd.host !== undefined) {
                assert.strictEqual(typeof config.logPropd.host, 'string',
                    'bad config: logPropd host must be a string');
                this.logPropd.host = config.logPropd.host;
            }
        }
