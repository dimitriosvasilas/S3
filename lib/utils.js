...
utils.normalizeRequest = function normalizeRequest(request) {
    ...
    request.queryStr = parsedUrl.query;
...
utils.isUnsupportedQuery = function isUnsupportedQuery(queryStr) {
    if (!queryStr) {
        return false;
    }
    // Reject unsupported query fields (e.g. ?accelerate), but allow
    // the same strings as values (e.g. ?test=accelerate) and queries like
    // '?acceleratefoobar' or '&foobaraccelerate'
    return constants.unsupportedQueries.some(q => {
        // ?accelerate, ?accelerate=[...], ?accelerate&[...]
        if (queryStr.startsWith(q)) {
            if (queryStr.charAt(q.length) === ''
                || queryStr.charAt(q.length) === '='
                || queryStr.charAt(q.length) === '&') {
                return true;
            }
        }
        // ?[...]&accelerate=[...] and ?[...]&accelerate&[...]
        if (queryStr.includes(`&${q}=`) || queryStr.includes(`&${q}&`)) {
            return true;
        }
        // ?[...]&accelerate
        if (queryStr.endsWith(`&${q}`)) {
            return true;
        }
        return false;
    });
